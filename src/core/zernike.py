...
def fit_zernike(wavefront, mask, R_out, center, max_order=23):
    ...
    if wavefront.shape != mask.shape:
        raise ValueError(f"Las dimensiones del frente de onda {wavefront.shape} no coinciden con la máscara {mask.shape}")
    ...
    try:
        ...
        if masked_base.shape[1] < masked_base.shape[0]:
            raise ValueError(f"Insuficientes puntos de datos ({masked_base.shape[1]}) para {masked_base.shape[0]} coeficientes")

        # Mínimos cuadrados para obtener coeficientes
        coeffs, residuals, rank, s = np.linalg.lstsq(masked_base.T, masked_wavefront, rcond=None)
        
        # Verificar que el ajuste fue exitoso
        if len(coeffs) != max_order:
            raise ValueError(f"Se esperaban {max_order} coeficientes, pero se obtuvieron {len(coeffs)}")

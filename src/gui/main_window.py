...
class FitsViewer(QMainWindow):
    def __init__(self):
        ...
        self.addToolBar(
            Qt.TopToolBarArea, self.toolbar
        )  # Asegurar que la barra de herramientas está en la parte superior
        ...
        self.roddier_action = QAction(
            load_icon_safe("icons/roddier.png", (28, 28)), "Test de Roddier", self
        )
        ...
        self.reset_action = QAction(
            load_icon_safe("icons/trash.png", (24, 24)), "Borrar", self
        )
        ...
        self.center_action = QAction(
            load_icon_safe("icons/center.png", (28, 28)), "Centrar", self
        )
        ...
        self.config_action = QAction(
            load_icon_safe("icons/settings.png", (50, 50)), "Configuración", self
        )
        ...
        self.intra_scroll.wheelEvent = lambda event: self.handle_wheel_event(
            event, self.intra_label
        )
        self.extra_scroll.wheelEvent = lambda event: self.handle_wheel_event(
            event, self.extra_label
        )
    ...
    def display_image(self, image_data, label):
        if image_data is not None:
            ...
            q_image = QImage(
                normalized_image.data, width, height, width, QImage.Format_Grayscale8
            )
    ...
    def run_roddier_test(self):
        ...
        if not self.intra_image_path or not self.extra_image_path:
            QMessageBox.warning(
                self,
                "Error",
                "Por favor, carga las imágenes intra y extra-focal primero.",
            )
            return

        try:
            roddier_dialog = RoddierTestDialog(
                self.intra_image_data, self.extra_image_data, crop_size=250
            )
            if roddier_dialog.exec_() == QDialog.Accepted:
                ...
                if telescope_params is None:
                    QMessageBox.warning(
                        self, "Error", "Error obteniendo parámetros del telescopio."
                    )
                    return
                ...
                if roddier_params is None:
                    QMessageBox.warning(
                        self,
                        "Error",
                        "Error obteniendo parámetros del test de Roddier.",
                    )
                    return
                ...
                if cropped_intra is None or cropped_extra is None:
                    QMessageBox.warning(
                        self, "Error", "No se pudieron obtener las imágenes recortadas."
                    )
                    return
                ...
                if apertura <= 0 or focal <= 0 or pixel_scale <= 0:
                    QMessageBox.warning(
                        self,
                        "Error",
                        "Los parámetros del telescopio deben ser valores positivos.",
                    )
                    return

                self._interferogram_params = interferogram_params
                self._telescope_params = telescope_params

                self.worker = RoddierWorkerThread(
                    cropped_intra, cropped_extra, telescope_params, roddier_params
                )

                # Create progress dialog
                self.progress_dialog = QProgressDialog(
                    "Procesando...", "Cancelar", 0, 100, self
                )
                self.progress_dialog.setWindowTitle("Calculando Test de Roddier")
                ...
                self.worker.finished.connect(self._on_roddier_finished)
                self.worker.error.connect(self._on_roddier_error)
    ...
    def _on_roddier_finished(self, results):
        """Handle completion of Roddier calculations."""
        try:
            if hasattr(self, "progress_dialog"):
                self.progress_dialog.close()

            results_window = RoddierTestResultsWindow(
                "Resultados del Test de Roddier", self
            )
            results_window.img_avg = results["img_avg"]
            results_window.update_plots(
                zernike_coeffs=results["zernike_coeffs"],
                zernike_base=results["zernike_base"],
                annular_mask=results["annular_mask"],
                interferogram_params=self._interferogram_params,
                telescope_params=self._telescope_params,
                wavefront=results.get("wavefront"),
                wavelength_nm=results.get("wavelength_nm", 555),
            )
            results_window.show()

        except Exception as e:
            error_msg = f"Error mostrando resultados: {str(e)}"
            QMessageBox.critical(self, "Error", error_msg)
            import traceback

            traceback.print_exc()

    def _on_roddier_error(self, error_msg):
        """Handle errors from Roddier calculations."""
    ...
    def center_scroll_on_point(self, scroll_area, center_x, center_y):
        ...
        QTimer.singleShot(
            100, lambda: self._apply_scroll(scroll_area, scroll_x, scroll_y)
        )
